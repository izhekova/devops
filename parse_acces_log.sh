...
read -p "Please choose which result you want to see? 
       ...
       3) Mean response time per minute in seconds
...
Your choise here:" result
    case $result in
        ...
        3) awk '$6 == "\"GET"' $log_file | awk -F'[ ]' '{ print $4 $NF}' | awk -F '[:]' '{ print $2":"$3 " " $NF}' |awk '{count[$1]++;a[$1]+=$NF}END{for(i in a) print "Average " i,a[i]/1000000/count[i]}' | sort ;;
